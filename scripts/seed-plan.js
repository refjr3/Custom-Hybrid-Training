#!/usr/bin/env node
// Seed script: inserts all training weeks and days into Supabase.
// Uses the real schema: am_session, pm_session, note, is_race_day, is_sunday
//
// Usage:
//   SUPABASE_URL=... SUPABASE_SERVICE_KEY=... node scripts/seed-plan.js
// or with a .env file:
//   node --env-file=.env scripts/seed-plan.js
...
// ---------------------------------------------------------------------------
// Training data — mirrors the hardcoded BLOCKS in src/App.jsx
// ---------------------------------------------------------------------------
...
async function seed() {
  console.log(`Seeding ${ALL_WEEKS.length} weeks into Supabase...`);
  let weekOk = 0, dayOk = 0, errors = 0;

  for (const week of ALL_WEEKS) {
    const { days, ...weekRow } = week;

    // Upsert week — conflict on week_id
    ...
    if (weekErr) {
      console.error(`  ✗ week ${week.week_id}: ${weekErr.message}`);
      errors++;
      continue;
    }
    ...
    // Upsert days — conflict on (week_id, day)
    ...
    if (daysErr) {
      console.error(`  ✗ days for ${week.week_id}: ${daysErr.message}`);
      errors++;
    } else {
      dayOk += dayRows.length;
      console.log(`  ✓ ${week.block_id}/${week.week_id}  (${dayRows.length} days)`);
    }
  }

  console.log(`\nDone — ${weekOk} weeks, ${dayOk} days seeded. Errors: ${errors}`);

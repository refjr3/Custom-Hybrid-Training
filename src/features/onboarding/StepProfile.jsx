...
const DIVISIONS = [
  ...
  { value: "doubles_open", label: "Doubles Open" },
  { value: "doubles_pro", label: "Doubles Pro" },
  { value: "na", label: "N/A" },
];
...
export default function StepProfile({ value, onChange, onNext, saving, error }) {
  ...
  const canNext =
    ...
    (heightUnit === "cm"
      ? Number(value.height_cm) > 0
      : Number(ft) > 0 || Number(inch) > 0) &&
    (weightUnit === "kg" ? Number(value.weight_kg) > 0 : Number(value.weight_lbs) > 0) &&
    Boolean(value.hyrox_division) &&
    Boolean(value.training_experience);
  ...
  return (
    <GlassCard style={{ display: "flex", flexDirection: "column", gap: 14 }}>
      ...
      <label style={labelStyle}>
        HYROX division *
        <GlassSelect value={value.hyrox_division || ""} onChange={(e) => set("hyrox_division", e.target.value)}>
          <option value="" style={{ color: "#111" }}>
            Select…
          </option>
          {DIVISIONS.map((d) => (
            <option key={d.value} value={d.value} style={{ color: "#111" }}>
              {d.label}
            </option>
          ))}
        </GlassSelect>
      ...
      <label style={labelStyle}>
        Training experience *
        <GlassSelect value={value.training_experience || ""} onChange={(e) => set("training_experience", e.target.value)}>
          <option value="" style={{ color: "#111" }}>
            Select…
          </option>
          {EXPERIENCE.map((e) => (
...
const labelStyle = { display: "flex", flexDirection: "column", gap: 6, fontSize: 11, letterSpacing: "0.8px", color: "rgba(255,255,255,0.45)", textTransform: "uppercase" };
